...
import { GENDER_AUTH, GENDER_DEAUTH } from "../../../../Utils/Constant";
...
const AuthGender = () => {
  ...
  const selectedGender = useSelector(
    (state) => state.instaEnroll.genderSelectedItem
  );
  ...
  const isDeauthAllowed = selectedGender
    ? !userId.includes(
        selectedGender.created_userid && selectedGender.process_status === 2
      ) ||
      !userId.includes(
        selectedGender.created_userid_edited &&
          selectedGender.process_status === 5
      )
    : false;
  ...
  const handleAuthorize = () => {
    const payload = { gender_id: selectedGender.id };
    authorize(GENDER_AUTH, payload, "/body/gender/:id");
  };
  ...
  const handleDeauthorize = async () => {
    ...
    if (!showFeedback) {
      ...
      try {
        await deauthorize(GENDER_DEAUTH, payload, "/body/gender/:id");
      } catch (error) {
