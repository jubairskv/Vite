...
import { CATEGORY_AUTH, CATEGORY_DEAUTH } from "../../../../Utils/Constant";
...
const AuthCategory = () => {
  ...
  const isDeauthAllowed = selectedCategory
    ? !userId.includes(
        selectedCategory.created_userid && selectedCategory.process_status === 2
      ) ||
      !userId.includes(
        selectedCategory.created_userid_edited &&
          selectedCategory.process_status === 5
      )
    : false;
  ...
  const handleAuthorize = () => {
    const payload = { category_id: selectedCategory.id };
    authorize(CATEGORY_AUTH, payload, "/body/category/:id");
  };
  ...
  const handleDeauthorize = async () => {
    ...
    if (!showFeedback) {
      ...
      try {
        await deauthorize(CATEGORY_DEAUTH, payload, "/body/category/:id");
      } catch (error) {
