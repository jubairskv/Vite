...
import { OCCUPATION_AUTH, OCCUPATION_DEAUTH } from "../../../../Utils/Constant";
...
const AuthOccupation = () => {
  const navigate = useNavigate();

  const selectedOccupation = useSelector(
    (state) => state.instaEnroll.occupationSelectedItem
  );
  ...
  const isDeauthAllowed = selectedOccupation
    ? !userId.includes(
        selectedOccupation.created_userid &&
          selectedOccupation.process_status === 2
      ) ||
      !userId.includes(
        selectedOccupation.created_userid_edited &&
          selectedOccupation.process_status === 5
      )
    : false;
  ...
  const handleAuthorize = () => {
    const payload = { occupation_id: selectedOccupation.id };
    authorize(OCCUPATION_AUTH, payload, "/body/occupation/:id");
  };
  ...
  const handleDeauthorize = async () => {
    ...
    if (!showFeedback) {
      ...
      try {
        await deauthorize(OCCUPATION_DEAUTH, payload, "/body/occupation/:id");
      } catch (error) {
