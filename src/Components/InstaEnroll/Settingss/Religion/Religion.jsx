...
import { RELIGION_VIEW } from "../../../../Utils/Constant";
import { FaPlus } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import ReligionList from "./ReligionList";
import { useDispatch } from "react-redux";
import { setReligion } from "../../../../Redux/InstaEnrollSlice";
...
const Religion = () => {
  ...
  const { loading, error } = useFetchData(
    RELIGION_VIEW,
    (data) => {
      setReligionView(data?.religion_array || []);
      dispatch(setReligion(data?.religion_array || []));
    },
    setReligionView
  );
        ...
        <div className="flex justify-between items-center">
          ...
          {Add && ( // Render the Add button only if "Add" action is present
            <button
              ...
              onClick={() => handleNavigation("addreligion")}
            >
      ...
      {loading ? (
        ...
        <>
          {error ? (
            ...
            <ReligionList religion={religionView} />
          )}
        </>
      )}
    </div>
