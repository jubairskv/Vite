...
const ReligionForm = ({ selectedReligion, onSubmit, isSubmitting }) => {
  ...
  useEffect(() => {
    if (selectedReligion) {
      setReligionName(selectedReligion.religion_name);
      setDescription(selectedReligion.description);
    } else {
      setReligionName("");
      setDescription("");
    }
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      id: selectedReligion?.id, // Only pass id if editing
    };
  ...
  return (
    <div className="mt-4 ml-4 min-h-[80vh] mr-4 flex flex-col items-start">
      <h1 className="text-2xl font-bold text-gray-800 flex">
        {selectedReligion ? "Edit Religion" : "Create Religion"}
      </h1>
      <form
        ...
        <div className="flex flex-col sm:flex-row gap-8 justify-stretch">
          <div className="flex flex-col items-start w-full lg:w-96">
            ...
            <input
              ...
              id="religionName"
              value={religionName}
              onChange={(e) => setReligionName(e.target.value.toUpperCase())}
              ...
              placeholder={t("religion_placeholder_text")}
              required
              autoComplete="off"
            />
          </div>
