...
import { PROOF_OF_IDENTITY_VIEW } from "../../../../Utils/Constant";
import { FaPlus } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import ProofOfIdentityList from "./ProofOfIdentityList";
import { useDispatch } from "react-redux";
import { setProofOfIdentity } from "../../../../Redux/InstaEnrollSlice";
...
const ProofOfIdentity = () => {
  const [proofOfIdentityView, setProofOfIdentityView] = useState([]);
  ...
  const { loading, error } = useFetchData(
    PROOF_OF_IDENTITY_VIEW,
    (data) => {
      setProofOfIdentityView(data?.identity_proof_array || []);
      dispatch(setProofOfIdentity(data?.identity_proof_array || []));
    },
    setProofOfIdentityView
  );
        ...
        <div className="flex justify-between items-center">
          ...
          {Add && ( // Render the Add button only if "Add" action is present
            <button
              ...
              onClick={() => handleNavigation("addproofofidentity")}
            >
      ...
      {loading ? (
        ...
        <>
          {error ? (
            ...
            <ProofOfIdentityList proofOfIdentity={proofOfIdentityView} />
          )}
        </>
      )}
    </div>
