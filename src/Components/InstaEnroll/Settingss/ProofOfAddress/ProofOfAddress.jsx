...
import { PROOF_OF_ADDRESS_VIEW } from "../../../../Utils/Constant";
import { FaPlus } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import ProofOfAddressList from "./ProofOfAddressList";
import { useDispatch, useSelector } from "react-redux";
import { setAddressProof } from "../../../../Redux/InstaEnrollSlice";
...
const ProofOfAddress = () => {
  ...
  const { loading, error } = useFetchData(
    PROOF_OF_ADDRESS_VIEW,
    (data) => {
      setAddressProofView(data?.address_proof_array || []);
      dispatch(setAddressProof(data?.address_proof_array || []));
    },
    setAddressProofView
  );
        ...
        <div className="flex justify-between items-center">
          ...
          {Add && ( // Render the Add button only if "Add" action is present
            <button
              ...
              onClick={() => handleNavigation("addproofofaddress")}
            >
      ...
      {loading ? (
        ...
        <>
          {error ? (
            ...
            <ProofOfAddressList addressProof={addressProofView} />
          )}
        </>
      )}
    </div>
