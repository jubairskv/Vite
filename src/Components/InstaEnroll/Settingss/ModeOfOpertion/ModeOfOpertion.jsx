...
import { MODE_OF_OPERATION_VIEW } from "../../../../Utils/Constant";
import { FaPlus } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import ModeOfOperationList from "./ModeOfOpertionList";
import { useDispatch } from "react-redux";
import { setModeOfOperation } from "../../../../Redux/InstaEnrollSlice";
...
const ModeOfOperation = () => {
  const [modeOfOperationView, setModeOfOperationView] = useState([]);
  ...
  const { loading, error } = useFetchData(
    MODE_OF_OPERATION_VIEW,
    (data) => {
      setModeOfOperationView(data?.mode_of_operation_array || []);
      dispatch(setModeOfOperation(data?.mode_of_operation_array || []));
    },
    setModeOfOperationView
  );
        ...
        <div className="flex justify-between items-center">
          ...
          {Add && ( // Render the Add button only if "Add" action is present
            <button
              ...
              onClick={() => handleNavigation("addmodeofoperation")}
            >
      ...
      {loading ? (
        ...
        <>
          {error ? (
            ...
            <ModeOfOperationList modeOfOperation={modeOfOperationView} />
          )}
        </>
      )}
    </div>
