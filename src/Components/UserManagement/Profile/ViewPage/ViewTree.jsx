...
import TreeNode from "./ViewTreeNode";
import { createTreeDataFromModules } from "./ViewUtils";

const DynamicTree = ({ moduleData, menuActionsProfile, isEditable }) => {
  const dispatch = useDispatch();
  const [treeData, setTreeData] = useState([]);

  const menuActionslist = menuActionsProfile?.menu_actions || [];
  ...
  const updateRights = (nodes, nodeId, right, checked) => {
    return nodes?.map((node) => {
      ...
      // Check if all children have the right selected after the update
      if (updatedNode.children?.length > 0) {
        const allChildrenSelected = areAllChildrenSelected(
          updatedNode.children,
          right
        );
        updatedNode.selectedRights = {
          ...updatedNode.selectedRights,
          [right]: allChildrenSelected, // Set parent right based on children
        };
      }
  ...
  // Helper function to propagate right changes to all children
  ...
  // Helper function to check if all children have a specific right selected
  const areAllChildrenSelected = (children, right) => {
